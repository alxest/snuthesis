...
In verification, \cc{} verifies the soundness of an analysis pass modularly, independent from its clients.
Specifically,
...
%% This design is also fruitful because one can keep $match\_states$ relatively simpler.
...
This section presents intuition behind the three parameters (\Cref{sec:main-verification:instances}) with some interesting instances of them.
%% with their underlying ideas.
%% and try to convey the idea behind them.

The virtue of parameterizing $\MREL$ is that in the \caselabel{CALL} case of \Cref{fig:full-sim},
...
$\SREL$ is introduced to verify optimizations that change their symbol code, such as \code{Unusedglob}.
...
For \code{Unusedglob}, we use the following instance.
\[
\begin{array}{l@{}l}
...
Here, \code{d} means dropped identifiers, \code{s} source \Skel, \, and \code{t} target \Skel.
\code{statics\_of} means the list of identifiers marked as static variable and $s\code{@}i$ is a definition in $s$ whose identifier is $i$.
...
%% Note that if the dropped identifier is mapped, than  <--- src UB는 괜찮...

Similarly to the above, the virtue of parameterizing $\SREL$ is that in \caselabel{INIT} case of \Cref{fig:full-sim}, one can rely on $\cdashbox{green}{$(\msem_{src}.\texttt{senv}, \msem_{tgt}.\texttt{senv}) \in \simskenv(\srel, \mrel)$}$.
For this, a pair of the module first guarantees their symbol codes are related: \cfbox{green}{$(\module_{\src}.\texttt{scode}, \module_{\tgt}.\texttt{scode}) \in \simsk(\srel)$}.
Recall that in the loading process (\Cref{sec:main-semantics:loading}),
...
The idea behind $\MPRED$ is already discussed in \Cref{sec:main-verification:openprsv}.
...
%% introduced to verify optimizations employing static analyzers.
%% Open Preservation with Parameters
%% SoundTop
\[
\begin{array}{l@{}l}
(
...
The idea behind the two examples \Cref{sec:overview-verification:injection:dynamic} and \Cref{fig:flat-model} are similar: protection of private memory.
...
%% However, while the former in its essence requires relational reasoning so it should be verified with enriched memory injection,
%% the latter suffices to use unary reasoning, so it is verified memory identity together with open preservation, which greatly simplifies the proof.
%% While the idea behind two examples \Cref{sec:overview-verification:injection:dynamic} and \Cref{fig:flat-model} are similar -- protection of private memory --
