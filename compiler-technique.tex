...
$\SREL$ is introduced to verify optimizations that changes their symbol environments, such as \code{Unusedglob}.
...
We omit exact definition of $\simskenv$; it morally means that the injection does not map dropped identifier in the source and all other identifiers are mapped one-to-one.
%% Note that if the dropped identifier is mapped, than  <--- src UB는 괜찮...

Similarly with above, the virtue of parameterizing $\SREL$ is that in \caselabel{INIT} case of \Cref{fig:full-sim}, one can rely on $\cdashbox{green}{$(\msem_{src}.\texttt{senv}, \msem_{tgt}.\texttt{senv}) \in \simskenv(\srel, \mrel)$}$.
For this, a pair of module first guarantees their symbol codes are related: \cfbox{green}{$(\module_{\src}.\texttt{scode}, \module_{\tgt}.\texttt{scode}) \in \simsk(\srel)$}.
...
loaded into global symbol environment, and finally projected into individual symbol environments.
...
For instance, suppose that a module $A$ claims that a static identifier $i$ of another module $B$ is dropped while it actually isn't.
Then, the resulting injection will not map $i$, so the $\simskenv$ of $B$ is broken.
To prevent this, $\simsk$ requires that the identifiers one claims to be dropped should be its own \emph{static} variables ($\code{statics\_of } s$),
which prevents invalidating the invariants of other modules.
Then, such information is maintained throughout the linking process by $\sqsubseteq$ relation.



$\MPRED$ is introduced to verify optimizations employing static analyzers.
