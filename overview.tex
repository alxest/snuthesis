...
its returning state should satisfy the {\it postcondition} $Q$. Then, the following {\it sequence} (or sequential composition) rule is sound.
...
\noindent Thanks to this sequence rule, the verifier can modularly verify each instruction of the program and then compose them to establish whole program correctness.
\todo{probably need to say sth about while rule}
...
There are also some variants of Hoare logic which proves total correctness, but it is too stringent to be realistic. Consider the verification of library code where clients are unknown.
Using total Hoare logic here will rule out potential non-terminating clients (e.g., operating system).
Second, \todo{whole-program verif} it mandates all the modules to be verified with same Hoare logic...
...
%%% plcc 120p
%%% VC 40p
Modern higher-order variants of Hoare logic\cite{VST, appel:plcc} also supports yet another modular reasoning principle, but justifying it accompanies much complexity and it only works for partial correctness.
\[
\infer
...
The principle is seemingly unsound because it is a circular reasoning, but (with a little fix \todo{is this fix essential?}) it is indeed sound. %%because we are proving only the partial correctness.
...
\footnote{Actually, the principle is unsound for total correctness. For an instance, given $f \defeq g()$ and $g \defeq f()$, one would be able to prove \hoare{True}{f}{r . r = 42} and \hoare{True}{g}{r . r = 42}.
...
\todo{We don't have such drawbacks - total correctness, don't mandate user, no complexity from step-index, gradual abstraction, ... }
...
\todo{Iris jfp 도 full functional, contextual refinement 지원한다고 써놓기는 했는데 이게 total weakestpre라서 invariant 못쓸거임}
