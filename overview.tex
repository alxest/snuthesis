...
%
Third, compilers are {\it general}, which means that they are designed to serve arbitrary C code so their translations are relatively simpler.
However, in program verification, we write specifications for each C module so each abstraction is {\it special} which often require deeper understanding about its algorithmic nature.
...
Now we explain how we can apply RUSC to program verification with an example (\Cref{fig:program-verif}) where two mutually recursive modules f and g are verified against its specifications even in the presence of an unknown assembly module h.
...
Specifically, $f_{spec}$ calls $g$ -- which can be an arbitrary function because we are proving under an arbitrary context -- and then check if $g$ returns the expected value. If so, it will proceed, but if not, it will trigger UB.
As a result, when verifying $f_{spec} \rusc_\rels f_{c}$ one needs to proceed the simulation proof only when $g$ behaves as expected, because otherwise the proof is trivial.
After each module's verification is over, we can {\it merge} these modules to form $fg_{spec}$ and remove the potential source of UB.
...
Therefore, even though verifying $f_{c}$ againts $f_{spec}$ requires a specialized simulation, RUSC can ably support them.
...
We will give a more comprehensive comparison between higher-order separation logic in \Cref{sec:program:hoare} and discuss future research directions in \Cref{sec:program:future}.
