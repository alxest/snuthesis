...
Then, we point out that our approach does not suffer from the aforementioned problems in \Cref{sec:program:problem} while supporting the modular reasoning principle (\Cref{sec:program:solution:advantages}).
...
Our approach does not suffer from the aforementioned problems in \Cref{sec:program:problem} while supporting the modular reasoning principle (\Cref{sec:program:solution:advantages}).

First and foremost, by passively modeling the expected behavior of other modules with UB, we do not have any circularity in our reasoning.
Therefore, we do not need a step-index, so our specification is clean and easy to understand.
Furthermore, as our specification is written in operational semantics, it can even be executed and tested.
It is crucial to have a reliable and understandable specification, perhaps as important as the verification itself, and testing is widely adopted as a tool to establish such trust.
%
...
%
Third, we do not impose any restriction on the client module because RUSC quantifies over an arbitrary context.
%
...
Note that while we give terminating specifications for terminating implementations and reactive (non-terminating and consistently communicating with the outside world, like REPL) specification for reactive modules,
their correctnesses are all expressed in the same RUSC relation. This is in contrast with where they define Hoare triple for partial correctness and total correctness separately, and these two do not coexist.
...
They mostly consist of executing C (or assembly) semantics step-by-step, making a simple reduction on the local environment (or register state), and applying memory related lemmas appropriately.
Note that all of these are shared patterns among general C (or assembly) program verification.
We did not do any proof automation at all, so there is a big room for improvement.

For ``The Rest,'' \texttt{mutual-sum} and \texttt{utod} have quite different reasons for their SLOC.
In \texttt{utod}, the whole SLOC is about proving the self-relatedness of the specification module.
...
The main technical hassle that made this long SLOC is that we had to take unknown context into account.
