...
We first give a short introduction to Hoare logic (\Cref{sec:program:background}), discuss its problems (\Cref{sec:program:problem}), and present how we verify interesting examples without aforementioned problems (\Cref{sec:program:solution}).
However, RUSC-as-a-program-logic is still on its early stage so we clarify its current limitations, compare it with Hoare logic, and discuss future research directions (\Cref{sec:program:future}).
