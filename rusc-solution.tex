...
The key insight here is that by inserting {\it dummy passes} -- which does not modify anything -- {\it only one} module is actually translated between each row.
...
For an instance, between the first and second row:
\begin{itemize}
...
Actually, the idea of inserting dummy passes has already been employed in \scc{}'s ``Level B'' correctness, but their technique is akin to contextual refinement and thus RUSC can be seen a generalization of their technique.
