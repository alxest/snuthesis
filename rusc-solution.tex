...
\revision{and the notion of self-related context from \cite{stewart:ccc}
  (see \Cref{sec:compiler:related} for comparison).}
...
of value and memory (see \Cref{sec:compiler:background:isem} and \Cref{sec:compiler:semantics} for details).
...
variables of C (see \Cref{sec:compiler:advanced} for details).
...
prove their horizontal compositionality and adequacy (See \Cref{sec:compiler:technique} for details).
...
we could easily prove that \emph{all} Clight\footnote{Clight is taken
  ...
  (see \Cref{sec:compiler:compcertm}).
}
...
Second, we demonstrate, via small but interesting examples (see \Cref{sec:program:solution}),
